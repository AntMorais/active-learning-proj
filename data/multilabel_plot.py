...
def split_train_testLOL(test_size):
    # choose a dataset with unbalanced class instances
    data = make_multilabel_classification(
        n_samples=50, n_classes=5, allow_unlabeled=False)

    X = StandardScaler().fit_transform(data[0])
    Y = data[1]
    print(data)
    #X, Y = import_libsvm_sparse('dataset').format_sklearn()
...
def split_train_test(dataset_filepath, test_size, n_labeled):
    ...
    trn_ds = Dataset(X_train, np.concatenate(
        [y_train[:n_labeled].tolist(), [None] * (len(y_train) - n_labeled)]))
    tst_ds = Dataset(X_test, y_test.tolist())
...
def main():
    ...
    for i in range(10):  # repeat experiment
        #trn_ds, tst_ds, fully_labeled_trn_ds = split_train_test(dataset_filepath, test_size, n_labeled)
        
        trn_ds, tst_ds, fully_labeled_trn_ds = split_train_testLOL(test_size,)
