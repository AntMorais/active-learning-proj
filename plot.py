...
def run(trn_ds, tst_ds, lbr, model, qs, quota, metric):
    E_f1 = []
    auc_total = []
...
def main():
    #para calcular media (arrays utilizados comop acumuladores para depois poder calcular a media para grafico)
    E_f1_1_global, E_f1_2_global, E_f1_3_global, auc_global_1, auc_global_2, auc_global_3 = [], [], [], [], [], []
    #numero de vezes que corre
    n_sample = 3

    #Preencher de forma a aparecer no grafico 
    interest_metrica = str(sys.argv[1]) #'Weight', 'Max', 'Sum'
    titulo_dataset = str(sys.argv[2]) #'abalone', 'sat', 'wine', 'iris'

    # Specifiy the parameters here:
    # path to dataset
    #'winequality-white_libsvm.txt' , 'sat_datalibsvm.txt', 'abalonelibsvm.txt'
    dataset_filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)), titulo_dataset+'libsvm.txt')
    test_size = 0.40   # the percentage of samples in the dataset that will be randomly selected and assigned to the test set
    ...
    for _ in range(n_sample):
        ...
        # Comparing Interest(Euclidean) strategy with RandomSampling and UncertaintySampling
        #INTEREST(Euclidean)
        qs = UncertaintySampling(trn_ds, method='euclidean', model=LogisticRegression(), metric=interest_metrica)
    ...
    #METRICAS
    #F1 score - interest
    f = open(interest_metrica+"Interest"+titulo_dataset+"Dataset.txt", "a")
    f.write('F1 Score:\n')
    f.write('Interest: \n'+str(E_f1_1)+'\n')
    ...
    # Plot the learning curve of UncertaintySampling, RandomSampling and InterestSampling
    # The x-axis is the number of queries, and the y-axis is the corresponding F1Score and Auroc
    query_num = np.arange(1, quota + 1)
    
    #AUROC
    plt.figure()
    plt.plot(query_num, auc_total_1, 'g', label='Interest '+interest_metrica)
    plt.plot(query_num, auc_total_2, 'k', label='Random')
    plt.plot(query_num, auc_total_3, 'c', label='Entropy')
    
    plt.xlabel('Number of Queries')
    plt.ylabel('Area Under the Curve')
    plt.title('AUROC - '+ titulo_dataset)
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
               fancybox=True, shadow=True, ncol=5)

    #F1 SCORE
    plt.figure()
    plt.plot(query_num, E_f1_1, 'g', label='Interest '+interest_metrica)
    plt.plot(query_num, E_f1_2, 'k', label='Random')
    plt.plot(query_num, E_f1_3, 'c', label='Entropy')
    
    plt.xlabel('Number of Queries')
    plt.ylabel('F1 score')
    plt.title('F1 score - ' + titulo_dataset)
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
               fancybox=True, shadow=True, ncol=5)
    plt.show()

#funcao que faz plot do ROC
