...
class UncertaintySampling(QueryStrategy):
    ...
    def make_query(self, return_score=False):
        ...
        if(self.method == 'euclidean'):
            ...
            #interesse - METRICAS
            #substituir com o nome do dataset e metricas
            # f = open("/Users/pedrosa/Documents/MEI/1º ano/IA/Human in the loop/Open Set Recognition/active-learning-proj/data/test.txt", "a")
            # f.write("\nprot: \n"+ str(prot))
            ...
            for i in range(len(X_pool)):
                ...
                #uncertainty: -sum(p(new = pi)*log(p(new = pi))) (PROVAVEL MUDAR AQUI TAMBEM)
                prob_classes = []
            ...
            # f.write("\ninterest_unknow_incertainty : " + str(interest_unknow_incertainty))

            #reset no array
            interest_unknow_incertainty = []
            #confirmar as labels
            # f.write("\nthe one: " + str(X_pool[ask_id[0]]))
            
            # f.write("\ncom distancia euclideana:  " + str(euclidean_dist[ask_id[0]]))
            # f.write("\ncom interesses: " + str(interest_unknow_incertainty_array[ask_id[0]]))
            # print("supostamente igual a isto: (na outra linha nabo)")
            #f.close()
